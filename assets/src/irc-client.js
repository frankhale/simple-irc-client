  ...
  constructor() {
    ...
    this.commands = [
      ...
      },
      {
        ...
        func: (args) => {
          if(this.state.client !== undefined) {
            this.state.client.action(this.state.currentChannel, args.join(" "));
          }
        }
      },
      {
      ...
      },
      {
        ...
        func: (args) => {
          if(this.state.client !== undefined &&
              args.length === 1) {
            this.state.client.say("nickserv", `identify ${args[0]}`);
          }
        }
      },
      {
    ...
    this.eventHandlers = {
      names: (channel, nicks) => {
        ...
        if(this.state.currentChannel === channel) {
          ...
          this.printResponse(`<span>Users: ${coloredNicksHtml.join(", ")}</span><br/>`);
      ...
      topic: (channel, topic, nick, message) => {
        ...
        if(this.state.currentChannel === channel) {
          this.printResponse(`<span style='background-color: #aaa;'>${this.sanitizeHTMLTag(channel)}: ${this.sanitizeHTMLTag(topic)} set by ${nick}</span><br/>`);
          this.scrollContentArea();
        }
      },
